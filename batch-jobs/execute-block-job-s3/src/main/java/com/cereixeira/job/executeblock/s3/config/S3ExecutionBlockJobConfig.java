package com.cereixeira.job.executeblock.s3.config;
...
import com.cereixeira.job.executeblock.s3.reader.BlockItemReader;
import com.cereixeira.job.executeblock.s3.tasklet.GenerateExecutionTasklet;
import com.cereixeira.job.executeblock.s3.writer.BlockWriter;
...
@PropertySource(value = "classpath:execute_block-application.properties", ignoreResourceNotFound = true)
@PropertySource(value = "file:config/execute_block-application.properties", ignoreResourceNotFound = true)

@Import({ DatabasesConfig.class, S3ConnectorConfig.class
})

@ComponentScan(basePackages = "com.cereixeira.job.executeblock.s3")
public class S3ExecutionBlockJobConfig {
    ...
    @Bean(name = S3ExecuteBlockJobBeanName.EXECUTION_BLOCK_SIMPLE_JOB_LAUNCHER)
    ...
    @Bean(name = S3ExecuteBlockJobBeanName.EXECUTION_BLOCK_JOB)
    public Job ingestionExecutionAndBlockJob() throws Exception {
        return jobBuilderFactory.get(S3ExecuteBlockJobBeanName.EXECUTION_BLOCK_JOB)
    ...
    @Bean(name = S3ExecuteBlockJobBeanName.STEP1_GENERATE_EXECUTION)
    public Step step1GenerateExecution(){
        return stepBuilderFactory.get(S3ExecuteBlockJobBeanName.STEP1_GENERATE_EXECUTION)
                .tasklet(taskGenerateExecution())
                .build();
    }
    @Bean(name = S3ExecuteBlockJobBeanName.STEP2_GENERATE_BLOCK)
    public Step step2GenerateBlock() throws Exception {
        return stepBuilderFactory.get(S3ExecuteBlockJobBeanName.STEP2_GENERATE_BLOCK)
